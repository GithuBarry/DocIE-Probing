    ...
    with open(doc_file) as f_doc:
        for line in f_doc:
            line_json = json.loads(
                line)  # odict_keys(['char_before', 'char_end', 'char_start', 'dateline', 'docid', 'source', 'tags', 'text'])
                ...
                for entity in template[role]:
                    ...
                    for mention in entity:
                        if mention in doc_text_no_n:
                            ...
                            problematic_mention_docids.append([mention, docid])
                    mention_idxs_sorted = sorted(mention_idxs, key=lambda m_idx: m_idx[
                        1])  # rank mentions by mention's first appearence
                    # if mention_idxs != mention_idxs_sorted: import ipdb; ipdb.set_trace()
                    if mention_idxs_sorted:
                        entity_idxs.append(mention_idxs_sorted)

                try:
                    entity_idxs_sorted = sorted(entity_idxs, key=lambda e_idx: e_idx[0][
                        1])  # rank entitys by entity's first mention's first appearence
                except IndexError:
                    import ipdb;
                    ipdb.set_trace()
