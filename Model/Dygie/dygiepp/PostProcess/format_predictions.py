...
def run_with(raw_output_dir, muc_gtt_input_path, parsed_output_path, file_prefix):
    ...
    for file in os.listdir(raw_output_dir):
        if file[:len(file_prefix)] == file_prefix:
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        muc_gtt_input_path = "../../../../Corpora/MUC/muc/processed/" + (
            file if file != "validation" else "dev") + ".json"
        ...
        run_with(raw_output_dir=raw_output_dir, muc_gtt_input_path=muc_gtt_input_path,
                 parsed_output_path=parsed_output_path, file_prefix=file)
