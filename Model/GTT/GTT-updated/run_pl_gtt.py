...
from utils_gtt import convert_examples_to_features, read_examples_from_file, read_golds_from_test_file, not_sub_string
...
class NERTransformer(BaseTransformer):
    ...
    def test_step(self, batch, batch_nb):
        ...
        hidden_states = [(tensor.cpu().numpy()) for tensor in hidden_states]
        np.save("./first_last_layer_"+str(batch_nb)+"_time"+str(int(time.time())), np.array([hidden_states[0],hidden_states[-1]]) if hidden_states else [])
